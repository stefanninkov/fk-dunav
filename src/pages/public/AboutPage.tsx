...
export function AboutPage() {
  ...
  useEffect(() => {
    if (!active) return;
    const unsub = onSnapshot(contentPageDoc(active.id, 'oTurniru'), (snap) => {
  ...
  if (!active) {
    return <PagePlaceholder title={sr.nav.about} description="Čeka se aktivan turnir." />;
  }
    ...
    <section className="mx-auto max-w-[720px] px-page-x py-12 lg:px-page-x-lg">
      ...
      {page && page.body.trim() ? (
        ...
        <p className="mt-6 rounded-md bg-surface-1 px-4 py-6 text-sm text-ink-secondary">
          Uskoro — organizator će objaviti informacije o turniru.
        </p>
      )}
    </section>
  );
